...
import { followNewUser, unfollowUser, updateUser, updateUserThunk } from "../../store/session"
...
const FollowButton = ({ post }) => {
    ...
    return (
        <div>
            ...
            {following && post.user_id in following ?
                <div className="followed" onClick={_ => dispatch(unfollowUser(post.user_id))}>
                    <i id="follow-plus-icon" className="fa-solid fa-check"></i>
                    Following
                </div> :
                <div className="not-followed" onClick={_ => dispatch(followNewUser(post.user_id))}>
